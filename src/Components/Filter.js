...
class Filter extends Component {
   ...
   componentDidMount() {
        ...
        axios.get(`${API_URL}/getAllRest`)
            .then(result => {
                const locations = result.data.restaurants;
                const selectedCity = locations.find(city => city.city == city_id);
                const cityLocations = locations.filter(city => city.city == city_id);
                this.setState({
                    locations: result.data.restaurants,
                    selectedCityName: selectedCity.city_name,
                    locationsInCity: cityLocations,
                    selectedLocation: cityLocations[0].city
                });
                setTimeout(() => {
    ...
    getPages = () => {
        ...
        for (let i = 0; i < noOFPages; i++ ) {
            pages.push(<span key={i} onClick={() => this.handlePage(i+1,noOFPages)} className="paginationButton active">{ i+ 1 }</span>)
        }
        return pages;
    }
    ...
    render() {
        const { locations, selectedCityName, mealTypeName, locationsInCity, restaurantList, pageNo,noOFPages } = this.state;
        let currPage = pageNo;
        return (
            <>
                <div className="container">
                    ...
                    <div className="row">
                            ...
                            <select className="locationSelection" onChange={(e) => this.handleLocationChange(e)}>
                                {
                                    locationsInCity.map((item, index) => {
                                        return <option key={index} value={item.city}>{ item.locality }</option>
                                    })
                                }
                            </select>
                        ...
                        <div className="col-12 col-lg-8 col-md-8 resultSection">
                            {
                                        ...
                                        return <div key={index} className="row resultBox" onClick={() => this.goToDetails(item)}>
                                                    <div className="topSection row">
                                                        ...
                                                        <div className="col-9">
                                                            ...
                                                            <div className="resultSubHeader">{item.address}</div>
                                                            <div className="resultDescription">{item.city_name}</div>
                                                        </div>
                                                    </div>
                                                    <div className="bar1 my-4"></div>
                            ...
                            }
                            {
                                ...
                                ?
                                    <div className="paginationOptions">
                                        <span className="paginationButton" onClick={() => this.handlePage(--currPage,noOFPages)}>&#8592;</span>
                                        {
                                            this.getPages()
                                        }
                                        <span className="paginationButton" onClick={() => this.handlePage(++currPage,noOFPages)}>&#8594;</span>
